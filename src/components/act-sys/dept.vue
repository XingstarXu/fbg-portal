    ...
    <div>
        ...
        <b-modal
            ...
            <template
                ...
                :class="modal.form.hidden">
                <b-form-group
                    v-for="(item, index) in modal.dept_list"
                    :key="index"
                    class="font label"
                    :label="item.txt"
                    :label-for="item.id">
...
export default {
    data() {
        return {
            ...
            disable_sel_val: -1,
            urls: {
                cre_dept: "http://192.168.12.26:9090/act-sys/cre-dept/",
                sel_dept_for_table: "http://192.168.12.26:9090/act-sys/sel-dept-for-table/",
                upd_dept: "http://192.168.12.26:9090/act-sys/upd-dept/",
                gen_pdf: "http://192.168.12.26:9090/act-sys/gen-dept-pdf/",
                gen_excel: "http://192.168.12.26:9090/act-sys/gen-dept-excel/"
            },
            search: "",
            toast: {
                err_type: "danger",
    ...
    methods: {
            show_cre_modal: function() {
                ...
                this.modal.form.btn.func = this.modal.form.btn.cre_dept
                // 顥現按鈕
            ...
            show_upd_modal: function(item) {
                ...
                this.modal.form.btn.func = this.modal.form.btn.upd_dept
                // 填寫部門資料
            ...
            modal_btn_func: function(target) {
                if(target == this.modal.form.btn.cre_dept) {
                    this.cre_dept()
                }
                else if(target == this.modal.form.btn.upd_dept) {
                    this.upd_dept()
                }
            },
            cre_dept: function() {
            ...
            sel_dept_for_table: function(
                cur_page, search="", disable=-1, order_by, order_desc
            ) {
                let self = this
                self.axios.post(
                    self.urls.sel_dept_for_table,
                    {
                        page: cur_page,
            ...
            search_dept_by_disable: function(value) {
                this.sel_dept_for_table(
                    1,
                    this.search,
                    value,
            ...
            gen_excel: function() {
                ...
                self.axios(
                    ...
                    // 設罝 <a> 屬性值
                    link.href = f_url
                    link.setAttribute("download", "DEPT-DATA.xlsx")
                    // 把 <a> 加入網頁中
